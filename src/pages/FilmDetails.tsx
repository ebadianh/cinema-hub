  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!res2.ok)
          throw new Error(`Regissörer: ${res2.status} ${res2.statusText}`);
        if (!res3.ok)
          throw new Error(`Skådespelare: ${res3.status} ${res3.statusText}`);
        ...
        const resShowings = await fetch(`/api/showings`, {
          signal: controller.signal,
        });
        if (resShowings.ok) {
          const showingsData = await resShowings.json();
          const showingsList: Showing[] = Array.isArray(showingsData)
            ? showingsData
            : (showingsData.showings ?? []);
          const filmShowings = showingsList.filter((s) => s.film_id === filmId);
          setShowings(filmShowings);

          if (dateFromURL) {
            setSelectedDate(dateFromURL);
          } else if (filmShowings.length > 0) {
            const firstDate = filmShowings[0].start_time.split("T")[0];
...
*/
  ...
  if (error)
    return <div className="container mt-4 text-danger">Fel: {error}</div>;
      ...
      <div className="d-block d-md-none">
        ...
        <div className="p-4 h-100">
          ...
          <p className="text-muted mb-3">
            {film.production_year} | {film.genre} |{" "}
            {formatDuration(film.duration_minutes)} | {film.age_rating}+
          </p>
              ...
              <div className="d-flex flex-wrap gap-2 mb-3 fd-chip-row fd-chip-row--dates">
                {availableDates.map((date) => (
                  <button
                    key={date}
                    type="button"
                    className={`btn btn-sm fd-chip-btn ${
                      selectedDate === date ? "fd-chip-btn--active" : ""
                    }`}
                    onClick={() => {
