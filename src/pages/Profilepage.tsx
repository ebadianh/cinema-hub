...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!res.ok) throw new Error(`${res.status} ${res.statusText}`);
        const data = await res.json();
        const bookingList: Booking[] = Array.isArray(data)
          ? data
          : (data.bookings ?? []);
        ...
        const bookingsWithDetails = await Promise.all(
          bookingList.map(async (booking) => {
            const showingRes = await fetch(
              `/api/showings/${booking.showing_id}`,
              { signal: controller.signal },
            );
            const showing = await showingRes.json();

            const filmRes = await fetch(`/api/films/${showing.film_id}`, {
              signal: controller.signal,
            });
  ...
  const handleCancel = async (bookingRef: string, startTime: string) => {
    ...
    setBookings((prev) =>
      prev.filter((b) => b.booking_reference !== bookingRef),
    );
  };
  ...
  const upcomingBookings = bookings.filter(
    (b) => b.showing && new Date(b.showing.start_time) > now,
  );
            ...
            <div className="card mb-4 border-danger">
              ...
              <div className="card-body">
                <p className="text-muted">
                  Här kan admin-funktioner läggas till i framtiden.
                </p>
              </div>
            </div>
          )}
